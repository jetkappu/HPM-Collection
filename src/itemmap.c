...
ACMD(itemmap) {
	...
	if (!_strcmpi(info->command,"itemmap_p"))
		get_type = 1;
	else if (!_strcmpi(info->command, "itemmap_g"))
		get_type = 2;

	if ( (!message || !*message) ||
		( get_type == 0 &&
		(sscanf_s(message, "\"%99[^\"]\" %d", item_name, &amount) < 1) &&
		(sscanf_s(message, "%99s %d", item_name, &amount) < 1) )
	) {
	...
	if ( (!message || !*message) ||
		( get_type == 1 &&
		(sscanf_s(message, "\"%99[^\"]\" %d, %23[^\n]", item_name, &amount, party_name) < 2) &&
		(sscanf_s(message, "%99s %d %23[^\n]", item_name, &amount, party_name) < 2) )
	) {
